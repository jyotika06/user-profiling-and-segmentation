...
if uploaded_file is not None:
    ...
    features = st.multiselect("📊 Select features for clustering", num_cols)

    if features:
        n_clusters = st.slider("🔢 Choose number of clusters", 2, 10, 3)

        # Add a button to run clustering only when user clicks
        if st.button("🚀 Run Clustering"):
            ...
            if len(features) >= 2:
                ...
                sns.scatterplot(x=features[0], y=features[1], hue='Cluster', data=df, palette='viridis', legend=False)
                st.pyplot(fig2)
        ...
        st.info("👉 Select features to enable clustering.")
else:
